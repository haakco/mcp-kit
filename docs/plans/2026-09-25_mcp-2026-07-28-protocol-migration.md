...
| Task 11's "no live docs teach initialize, sessions, GET SSE, deprecated capabilities, or DCR" is satisfiable by updating the kit's own docs. | The `haakco-mcp-plugins` skill in the separate `skills` repo is live (`skills/haakco-mcp-plugins` → `versions/0010`), synced into **both** consumers, and teaches an `mcpkit` API that was never built: `Config.Stateless`, `mcpkit.Legacy.WithSessionHandshake()`, `mcpkit.NewHandle`, `mcpServer.Call`, `mcpmw/headers.go`, `Config.Headers`, `HeaderMismatchError`, `mcpkit.MRTR.NewRequestState`, `mcpkit.Cache`. Its `metadata.yaml` advertises "MRTR, server/discover, subscriptions/listen, standard MCP headers" and "Deprecated items still kept during the deprecation window". Its "Concrete Stale Anchors" list points at `testkit/handshake.go` (deleted) and cites line ranges in files that were rewritten. | **Open decision, not actioned.** This is a design disagreement, not a typo: the skill prescribes dual-era support and keeping deprecated capabilities through the window, while this plan deliberately chose modern-only and removed the deprecated `logging` capability. Correcting it means authoring skill `versions/0011` and moving the symlink, which reverses someone's deliberate guidance — an owner decision, not a migration side effect. Needs a row here resolving `AOA task 6`-style deferrals against the shipped kit. |
...
- [ ] No live docs/fixtures teach initialize, sessions, GET SSE, deprecated capabilities, or DCR as primary. **Kit
      docs and consumer code are clean. Not satisfied across the org:** the live `haakco-mcp-plugins` skill (repo
      `skills`, symlinked to `versions/0010`, synced into both consumers) still teaches removed session/legacy paths
      and an `mcpkit` API that was never built. See the corresponding row in §1.1 — resolving it is an owner decision
      because it reverses deliberate dual-era guidance.
...
|---|---|---|---|---|
| mcp-kit release candidate | Branch head `b03cdfe` on `feat/mcp-2026-07-28-migration` (8 commits over `d86fb98`): `8101108` (migration), `9973cf3` (CI fix), `2d4d431` (Dependabot), `07980ed`, `95e4962`, `5240acc`, `b64823e`, `b03cdfe` (docs/plan); PR [#5](https://github.com/haakco/mcp-kit/pull/5) (draft) | `just quality` (build, vet, deep lint 0 issues, structural lint 0 issues, race — 13 packages), `just vulncheck` (no vulnerabilities), `just conformance` (CLI `0.2.0-alpha.11`, `--requirements 2026-07-28`, 164 passed / 0 failed scored, 0 baseline entries), negative control `-stateless=false` exits 1, `actionlint` clean, suite-selection logic verified under `/bin/dash`, `git diff --check` clean | **Green** on run [36123660998](https://github.com/haakco/mcp-kit/actions/runs/36123660998): Detect changes pass, Quality pass (4m55s), Conformance pass (1m54s), GitGuardian pass. First run failed and correctly skipped the suites; root cause was `set -o pipefail` under dash, fixed in `9973cf3`. Re-confirmed **Green** at head `b03cdfe` on run [36139282659](https://github.com/haakco/mcp-kit/actions/runs/36139282659) (Detect changes pass, Conformance pass, Quality pass; `conclusion=success`), which is the full PR diff, code commits included. | n/a |
...
| skills-mcp v0.6.0 | Repinned to `github.com/haakco/mcp-kit v0.6.0` on `feat/mcp-2026-07-28-migration`: commits `bea4fae0` (repin) and `13cdcb25` (merge of `origin/main` `ae68bc2b`, whose conflict was confined to `go.mod`/`go.sum` and was resolved by regenerating those files rather than hand-editing). No local `replace`; `go mod tidy` clean. PR [haakco/skills#166](https://github.com/haakco/skills/pull/166) went from `CONFLICTING` to **`MERGEABLE`**. | **Full** `SKILLS_TEST_POSTGRES_URL=… go test -p 1 ./... -count=1` → `test_rc=0`, 36 packages, zero failures, on the merged tree; `go build ./...` and `go vet ./...` both `rc=0`. Postgres 18 on `127.0.0.1:15432`. | Branch run [36151495833](https://github.com/haakco/skills/actions/runs/36151495833) **red — and this was this migration's regression, not infrastructure.** I first reported it as a pre-existing runner failure after reading only the job log tail (`Executing the custom container implementation failed`). **That was wrong.** The real failure sits above it: `[phase] initialize-mcp-session` / `Failed to get session ID.` — the Dagger `mcp-postgres-checks` verifier still did the retired `initialize` handshake. `main` failing on 2026-09-16 with the same runner string was a separate genuine flake; I over-generalised from it. CORRECTED and **RESOLVED** by [haakco/skills#168](https://github.com/haakco/skills/pull/168), **merged** as `220f06c`, which migrated the five stale non-Go MCP clients (`verify_postgres_mcp.sh`, four Playwright specs/fixtures, and `mcp_validate_common.py` plus its five callers, and `tooling/evals/driver.py`). CI is now fully green: `Backend Checks` pass, **`MCP/Postgres` pass**, **Playwright E2E 1/4–4/4 all pass**, plus `Web Static/Unit/Build`, `Docker Images (skills-mcp)` and `(skills-edge)`, `Helm Chart`, `Skills library`, GitGuardian; `mergeStateStatus=CLEAN`. Two things this episode teaches, both recorded in `haakco-mcp-plugins` 0012: migrating a server is not migrating its clients, and the ARC container error is an outer wrapper whose real failure is earlier in the log. Also: a first `Backend Checks` rerun was needed because the job failed *after* `skills backend/static checks passed` — in the `Post Run actions/checkout` cleanup step with `Unexpected server response: 500`, the genuine runner flake. Triage correction posted to [#166](https://github.com/haakco/skills/pull/166#issuecomment-5835411307). | |
